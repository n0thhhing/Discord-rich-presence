...
class RichPresence {
  constructor({ token, clientId, logs, handle }) {
    ...
    if (this.logs) {
      console.log(
        `${chalk.blue("[LOGS]")} - RichPresence logs are ${chalk.red(
          "Enabled",
        )}.` +
          `\nTo disable logs, put \n${chalk.blue(`logs`)}: ${chalk.red(
            "false",
          )}${
            this.handle
              ? ` and ${chalk.blue("handle")}: ${chalk.red("false")}`
              : ""
      ...
      if (this.handle) {
        console.log(
          `${chalk.blue(
            "[LOGS]",
          )} - RichPresence handle logs are also ${chalk.red("Enabled")}.`,
        );
      }
      console.log("");
    }
    ...
    if (this.websocket) {
      try {
        if (this.websocket.readyState === WebSocket.OPEN) {
          ...
          if (this.logs) {
            console.log(
              "WebSocket is still connecting. Waiting for connection to open...",
            );
          }
  ...
  handleRequestGuildMembers(data) {
    try {
      ...
      if (this.websocket.readyState === WebSocket.OPEN) {
        ...
        this.handleError(
          "WebSocket is not open. Unable to send guild members.",
        );
      }
    } catch (error) {
  ...
  handleDispatch(data) {
    ...
    if (type === 1) {
      ...
      if (this.logs) console.log(`Button with custom_id ${custom_id} clicked`);
    }
  }

  /**
   * Updates the presence on Discord.
   *
   * @param {Object} presenceData - Updated presence data.
   * @returns {Promise<void>} - Resolves when the presence update is sent successfully.
   */
  updatePresence(presenceData) {
    return new Promise((resolve, reject) => {
      try {
        if (this.logs) {
          console.log(chalk.blue("LOGS - ") + "Updating presence...");
        }
        ...
        if (this.connected && this.websocket.readyState === WebSocket.OPEN) {
          ...
          if (this.logs) {
            const code = JSON.stringify(payload.d);
            fs.writeFileSync("code.json", code);
            exec("prettier -w code.json", () => {
              const outcode = fs.readFileSync("code.json", "utf-8");
              console.log(syntaxHighlight(outcode));
              fs.unlink("code.json", (error) => {
                if (error) {
                  this.handleError("Error updating presence:", error);
                  reject(error);
                } else {
                  resolve();
                }
          ...
          this.handleError("WebSocket is not open. Unable to update presence.");
          reject("WebSocket is not open. Unable to update presence.");
        }
      } catch (error) {
        this.handleError("Error updating presence:", error);
        reject(error);
            ...
            if (activity.assets) {
              if (activity.assets.large_image) {
                if (this.isValidUrl(activity.assets.large_image)) {
                  activity.assets.large_image = this.replaceImageURL(
                    activity.assets.large_image,
                  );
                } else {
                  console.log(
                    "invalid url, please make sure to add a valid url to your presence data.",
                  );
                }
              }
              if (activity.assets.small_image) {
                if (this.isValidUrl(activity.assets.large_image)) {
                  activity.assets.small_image = this.replaceImageURL(
